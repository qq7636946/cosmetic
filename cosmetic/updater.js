...
const file_path = 'g:\\\\我的雲端硬碟\\\\2025_lee\\\\+++++專案\\\\++++++核點創意Nudot\\\\web_site\\\\nd\\\\index.html';
...
content = content.replace(/ *:root *\{[^}]*\}/s, root_new);
...
`;
content = content.replace(/\/\* 主視覺區塊 \*\/.*?\/\* --- 新增：第二區塊 \(Works\) 樣式 --- \*\//s, new_css + '\n    /* --- 新增：第二區塊 (Works) 樣式 --- */');

// 4. Remove old media query mobile adjustments for main/hero
content = content.replace(/ \+\/\* 主標題 \*\/.*?(?= \+\/\* Works section \*\/)/s, '');
content = content.replace(/ \+\/\* --- Hero --- \*\/.*?(?= \+\/\* --- Works section --- \*\/)/s, '');
content = content.replace(/ +\.hero-title \{.*?\.works-heading/s, '      .works-heading');

// 5. Replace <main> block
const new_main = `  <main class="hero-container">
...
content = content.replace(/<main>.*?<\/main>/s, new_main);

// 6. Replace GSAP code
const new_gsap = `      // 1. 頂部導覽列淡出(因為沒在使用，或保留作後續使用)
      tl.from(".title-text", {
          y: 100,
          opacity: 0,
          stagger: 0.2,
...
content = content.replace(/      \/\/ 1\. 頂部導覽列淡入.*?ease: "power2\.out"\n *\}, "-=1\.2"\);/s, new_gsap);

// 7. Remove Parallax Effect part of hero that is no longer valid
content = content.replace(/      \/\/ 滑鼠移動視差效果 \(Parallax Effect\).*?\/\/ --- 修改：滾動 Zoom Out 無痕銜接動畫 ---/s, '      // --- 修改：滾動 Zoom Out 無痕銜接動畫 ---');

// update scroll animation reference
content = content.replace('gsap.to("main"', 'gsap.to(".hero-container"');
